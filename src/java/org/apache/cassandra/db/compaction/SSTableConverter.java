...
public class SSTableConverter
{
    ...
    public SSTableConverter(ColumnFamilyStore cfs, LifecycleTransaction txn, OutputHandler outputHandler, String version)
    {
        this.cfs = cfs;
        ...
        this.controller = new ConversionController(cfs);
    ...
    private SSTableWriter createCompactionWriter(long repairedAt)
    {
        ...
        return SSTableWriter.create(Descriptor.fromFilename(cfs.getSSTablePath(directory, version)),
                                    estimatedRows,
                                    repairedAt,
                                    cfs.metadata,
    ...
    public Collection<SSTableReader> convert(boolean keepOriginals)
    {
        Collection<SSTableReader> readers = new ArrayList<>();
        outputHandler.output("Converting " + sstable + " to " + version);
             ...
             CompactionIterator iter = new CompactionIterator(transaction.opType(), scanners.scanners, controller, nowInSec, UUIDGen.getTimeUUID()))
        {
            ...
            readers = writer.finish();
            outputHandler.output("Conversion of " + sstable + " complete.");
        }
        catch (Exception e)
        {
    ...
    private static class ConversionController extends CompactionController
    {
        public ConversionController(ColumnFamilyStore cfs)
        {
