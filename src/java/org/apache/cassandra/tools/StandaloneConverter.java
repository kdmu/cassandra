...
{
    private static final String TOOL_NAME = "sstableconvert";
    ...
    {
        Options options = Options.parseArgs(args, false);
        ...
        try
        {
            ...
            for (Map.Entry<Descriptor, Set<Component>> entry : lister.list().entrySet())
            {
                ...
                try
                {
                    ...
                    if (sstable.descriptor.version.equals(DatabaseDescriptor.getSSTableFormat().info.getVersion(options.version)))
                        continue;
                    readers.add(sstable);
                }
                catch (Exception e)
            ...
            handler.output("Found " + numSSTables + " sstables that need conversion.");
    ...
    protected static class Options
    {
        ...
        public static Options parseArgs(String cmdArgs[], boolean isLatestVersion)
        {
            ...
            try
            {
                ...
                {
                    printUsage(options, isLatestVersion);
                    System.exit(0);
                }
                ...
                if (isLatestVersion)
                {
                    ...
                    if (args.length >= 4 || args.length < 2)
                    {
                        ...
                        errorMsg(msg, options, isLatestVersion);
                        System.exit(1);
                    }
                    ...
                    Options opts = new Options(keyspace, cf, DatabaseDescriptor.getSSTableFormat().info.getLatestVersion().toString(), snapshot);
            ...
            {
                errorMsg(e.getMessage(), options, isLatestVersion);
                return null;
            }
        }

        private static void errorMsg(String msg, CmdLineOptions options, boolean isLatestVersion)
        {
        ...
        public static void printUsage(CmdLineOptions options, boolean isLatestVersion)
        {
            if (isLatestVersion)
            {
                String usage = String.format("%s [options] <keyspace> <cf> [snapshot]", "sstableupgrade");
            ...
            else
            {
                ...
                StringBuilder header = new StringBuilder();
                header.append("--\n");
                header.append("Convert the sstables in the given cf (or snapshot) to the specified version of Cassandra.");
                header.append("This operation will rewrite the sstables in the specified cf.");
                header.append("The snapshot option will only convert the specified snapshot.");
                header.append("This will replace the files in the given snapshot as well as break any ");
                header.append("hard links to live sstables.");
                header.append("\n--\n");
                header.append("Options are:");
